...
export default function SavedItemsPage() {
  ...
  const generateWhatsAppMessage = () => {
    ...
    const totalValue = wishlistItems.reduce((sum, product) => sum + product.price, 0);
    const formattedTotal = `₦${totalValue.toLocaleString('en-NG')}`;

    // Build product list with proper formatting
    ...
    const message = `Hi VCSolar! 👋 I've picked out these items for my solar setup:%0A%0A${productList}%0A%0ATotal Value: ${formattedTotal}%0A%0ACan you confirm availability and send payment details so I can secure these?`;
    ...
    // Note: %0A is already in the message for proper line breaks
  ...
  const generateEmailLink = () => {
    ...
    const subject = `Order Inquiry for ${wishlistItems.length} Items - VCSolar Wishlist`;

    const productList = wishlistItems
      .map((product) => `${product.name} by ${product.brand} - ₦${product.price.toLocaleString('en-NG')}`)
      .join('\n');

    const body = `Hello VC Solar Team,\n\nI have selected the following items for my solar setup and would like to proceed with an order:\n\n${productList}\n\nTotal Value: ${formattedTotal}\n\nPlease confirm which of these are currently in stock and provide your payment details so I can secure this order.\n\nThank you!`;
        ...
        {wishlistItems.length > 0 && (
          <div className="flex flex-col sm:flex-row gap-3 mb-xl">
            <a
              ...
              className="flex items-center justify-center gap-2 px-6 py-3 min-h-[48px] bg-primary text-surface text-sm font-semibold rounded-lg hover:bg-primary-dark transition-colors shadow-md"
            >
